...
export default {
  async fetch(request, env, ctx) {
    ...
    // B. URL Shortener GET Route (Redirect 302 to long URL)
    // Dynamic short IDs are usually 5-8 alphanumeric characters
    const shortIdMatch = pathname.match(/^\/([a-zA-Z0-9]{5,8})$/);
    if (shortIdMatch && request.method === "GET") {
      const shortId = shortIdMatch[1];
      const reservedKeywords = ["about", "terms", "privacy", "contact", "faq", "assets", "api"];
      if (!reservedKeywords.includes(shortId)) {
        if (env.QRYTUBE_DATA) {
          const originalUrl = await env.QRYTUBE_DATA.get(shortId);
          if (originalUrl) {
    ...
    // C. URL Shortener POST Route (Saves URL to KV Namespace QRYTUBE_DATA)
    if (pathname === "/api/shorten" && request.method === "POST") {
      try {
        const body = await request.json();
        const originalUrl = body.url;
        if (!originalUrl) {
          return new Response(JSON.stringify({ error: "Missing long URL (url parameter)" }), {
            status: 400,
            headers: {
        ...
        // Save to KV Namespace QRYTUBE_DATA
        if (env.QRYTUBE_DATA) {
          await env.QRYTUBE_DATA.put(shortId, originalUrl);
        } else {
          return new Response(JSON.stringify({ 
            error: "Database error: QRYTUBE_DATA KV Namespace is not bound in wrangler.toml or Cloudflare dashboard." 
          }), {
            status: 500,
            headers: {
              "Content-Type": "application/json",
